...
import { TruckDTO } from '../models/truck.model'
...
interface TrucksState {
	...
	setUpdatedTruck: (truck: TruckDTO | null) => void
...
export const useTrucksStore = create<TrucksState>(set => ({
	...
	getTrucks: async (payload: RequestModel) => {
		...
		try {
			...
			set({
				...
				truckLength: response.meta.totalItems,
	...
	setUpdatedTruck: (truck: TruckDTO | null) =>
		set(state => ({
			trucks: state.trucks.map(t => (t._id === truck?._id ? truck : t)),
		})),
}))
