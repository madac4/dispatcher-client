...
import { TrailerDTO } from '../models/trailer.model'
...
interface TrailersState {
	...
	setUpdatedTrailer: (trailer: TrailerDTO | null) => void
...
export const useTrailersStore = create<TrailersState>(set => ({
	...
	getTrailers: async (payload: RequestModel) => {
		...
		try {
			...
			set({
				...
				trailerLength: response.meta.totalItems,
	...
	setUpdatedTrailer: (trailer: TrailerDTO | null) =>
		set(state => ({
			trailers: state.trailers.map(t =>
				t._id === trailer?._id ? trailer : t,
			),
		})),
}))
