...
import { NHTSA, TruckDTO, TruckPayload } from '../models/truck.model'
...
): Promise<PaginationResponse<TruckDTO>> => {
	return apiRequestPaginated<TruckDTO>(() =>
		api
			.get(
...
export const updateTruck = async (
	...
	return apiRequest(() =>
		api.put(`${baseURL}/${id}`, truck).then(res => res.data),
	)
}
...
export const uploadTruckFile = async (
	file: File,
	id: string,
...
export const getTruckFiles = async (
	id: string,
...
export const deleteTruckFile = async (
	id: string,
	filename: string,
): Promise<ApiResponse<null>> => {
	return apiRequest<null>(() =>
...
export const downloadTruckFile = async (
	id: string,
	filename: string,
): Promise<ApiResponse<null>> => {
	try {
