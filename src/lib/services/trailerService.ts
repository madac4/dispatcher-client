...
import { Trailer, TrailerPayload } from '../models/trailer.model'
...
export const getPaginatedTrailers = async (payload: RequestModel): Promise<PaginationResponse<Trailer>> => {
  return apiRequestPaginated<Trailer>(() =>
    api
...
export const getTrailerById = async (id: string): Promise<ApiResponse<Trailer>> => {
  return apiRequest(() => api.get(`${baseURL}/${id}`).then(res => res.data))
}

export const createTrailer = async (trailer: TrailerPayload): Promise<ApiResponse<Trailer>> => {
  return apiRequest<Trailer>(() => api.post(baseURL, trailer).then(res => res.data))
}

export const updateTrailer = async (trailer: TrailerPayload, id: string): Promise<ApiResponse<Trailer>> => {
  return apiRequest<Trailer>(() => api.put(`${baseURL}/${id}`, trailer).then(res => res.data))
}
...
export const uploadTrailerFile = async (file: File, id: string): Promise<ApiResponse<null>> => {
...
export const getTrailerFiles = async (id: string): Promise<ApiResponse<FileDTO[]>> => {
  return apiRequest<FileDTO[]>(() => api.get(`${baseURL}/${id}/files`).then(res => res.data))
}

export const deleteTrailerFile = async (id: string, filename: string): Promise<ApiResponse<null>> => {
  return apiRequest<null>(() => api.delete(`${baseURL}/${id}/files/${filename}`).then(res => res.data))
}

export const downloadTrailerFile = async (id: string, filename: string): Promise<ApiResponse<null>> => {
  try {
