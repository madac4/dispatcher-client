...
import { formatStatus, getStatusBadge, OrderStatus, PaginatedOrderDTO } from '@/lib/models/order.model';
...
export const truckColumns: ColumnDef<TruckDTO>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => <div className="font-mono">{row.getValue('licencePlate')}</div>,
  },
  {
    accessorKey: 'state',
    header: 'State',
    cell: ({ row }) => <div className="text-gray-600">{row.getValue('state')}</div>,
  },
  {
    accessorKey: 'make',
    header: 'Make/Model',
  ...
  },
  {
    ...
    header: 'Axles',
    cell: ({ row }) => <div className="font-mono">{row.getValue('nrOfAxles')}</div>,
  },
  {
    id: 'actions',
    enableHiding: false,
    cell: ({ row }) => {
      ...
      const actions: TableAction<TruckDTO>[] = [
        ...
        },
        {
          ...
          dialogContent: (item) => <EditInventoryEntity entity={item} type="truck" />,
        },
        {
          label: 'Attach Files',
          icon: 'paperclip',
          dialogContent: (item) => <UploadEntityFile type="truck" entityId={item._id} />,
        ...
        },
        {
          label: 'Check Files',
          icon: 'file-search',
          dialogContent: (item) => <FilesActionModal type="truck" id={item._id} />,
          action: () => {},
        },
        {
          label: 'Delete Truck',
          ...
          dialogContent: (item) => (
            <ConfirmModal
              ...
              onConfirm={async () => {
                const loading = toast.loading('Deleting truck...');
...
export const trailerColumns: ColumnDef<TrailerDTO>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div>
        ...
        <div className="text-sm text-gray-500">{row.original.length}</div>
      </div>
    ),
  },
  {
  ...
  },
  {
    accessorKey: 'vin',
    header: 'VIN',
    cell: ({ row }) => <div className="text-gray-600">{row.original.vin.slice(0, 8)}...</div>,
  },
  {
    accessorKey: 'state',
    header: 'State',
    cell: ({ row }) => <div className="text-gray-600">{row.original.state}</div>,
  },
  {
    accessorKey: 'nrOfAxles',
    header: 'Axles',
    cell: ({ row }) => <div className="text-gray-600">{row.original.nrOfAxles}</div>,
  },
    ...
    cell: ({ row }) => {
      ...
      const actions: TableAction<TrailerDTO>[] = [
        ...
        },
        {
          ...
          dialogContent: (item) => <EditInventoryEntity entity={item} type="trailer" />,
        },
        {
          label: 'Attach Files',
          icon: 'paperclip',
          dialogContent: (item) => <UploadEntityFile type="trailer" entityId={item._id} />,
          isDisabled: (item) => !item.unitNumber,
          action: (item) => {
            toast.info(`File attachment for truck ${item.unitNumber} not implemented`);
          },
        },
        {
          label: 'Check Files',
          icon: 'file-search',
          dialogContent: (item) => <FilesActionModal type="trailer" id={item._id} />,
          action: () => {},
        },
        {
          ...
          dialogContent: (item) => (
            <ConfirmModal
              ...
              onConfirm={async () => {
                const loading = toast.loading('Deleting trailer...');
...
export const orderColumns: ColumnDef<PaginatedOrderDTO>[] = [
  {
    ...
    cell: ({ row }) => (
      <div className="font-medium">
        <Link href={`/dashboard/orders/${row.getValue('orderNumber')}`} className="hover:underline text-primary">
  ...
  },
  {
    ...
    cell: ({ row }) => <div className="font-mono">{row.original.createdBy}</div>,
  },
  {
    ...
    cell: ({ row }) => <div className="font-mono">{format(row.getValue('createdAt'), 'dd MMM yyyy | HH:mm')}</div>,
  },
  {
    accessorKey: 'originAddress',
    header: 'Origin Address',
    cell: ({ row }) => <div className="text-gray-600">{row.original.originAddress}</div>,
  },
  {
    accessorKey: 'destinationAddress',
    header: 'Destination Address',
    cell: ({ row }) => <div className="text-gray-600">{row.original.destinationAddress}</div>,
  },
  {
    accessorKey: 'truckId',
    header: 'Truck',
    cell: ({ row }) => <div className="text-gray-600">{row.original.truckId}</div>,
  },
  {
    accessorKey: 'status',
    header: 'Status',
    cell: ({ row }) => {
      ...
      return <Badge className={getStatusBadge(status)}>{formatStatus(status)}</Badge>;
    },
  },
