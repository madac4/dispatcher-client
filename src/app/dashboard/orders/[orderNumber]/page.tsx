...
export default function OrderDetailsPage() {
  ...
  const downloadCarrierFile = async (filename: string) => {
    try {
      const { message } = await SettingsService.downloadCarrierFile(filename, order!.userId);
      toast.success(message);
    } catch (error) {
      console.error('Error downloading file:', error);
      toast.error('Failed to download file');
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {(role() === UserRole.ADMIN || role() === UserRole.MODERATOR) && !order.moderator && (
            <Button size="md" onClick={() => moderateOrder(order.id)} disabled={moderating}>
              {moderating ? <Loader2 className="w-4 h-4 animate-spin" /> : <UserCheck size="16" />}
              Moderate Order
            </Button>
          )}
          <Dialog open={isStatusDialogOpen} onOpenChange={setIsStatusDialogOpen}>
            ...
            <DialogContent className="sm:max-w-md">
                      ...
                      <SelectGroup>
                        {statuses.map((status) => (
                          <SelectItem key={status.value} value={status.value}>
                            {status.label}{' '}
                            <span className="text-primary">{status.value === order.status ? '(Current)' : ''}</span>
                          </SelectItem>
                        ))}
                      </SelectGroup>
                    </SelectContent>
              ...
              <DialogFooter className="sm:justify-start">
                ...
                >
                  {statusChanging ? <Loader2 className="w-4 h-4 animate-spin" /> : <Zap size="16" />}
                  Update Status
                </Button>
              </DialogFooter>
            </DialogContent>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-gray-50 rounded-lg p-3 text-center">
                    ...
                    <p className="font-bold text-gray-900">{formatDimensions(order.lengthFt, order.lengthIn)}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3 text-center">
                    <p className="text-xs font-medium text-gray-600 mb-1">WIDTH</p>
                    <p className="font-bold text-gray-900">{formatDimensions(order.widthFt, order.widthIn)}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3 text-center">
                    <p className="text-xs font-medium text-gray-600 mb-1">HEIGHT</p>
                    <p className="font-bold text-gray-900">{formatDimensions(order.heightFt, order.heightIn)}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3 text-center">
                ...
                <Table>
                  ...
                  <TableFooter>
                    <TableRow className="bg-primary-50 font-semibold">
                      ...
                      >
                        {order.axleConfigs.reduce((sum: number, config) => sum + config.weight, 0).toLocaleString()} lbs
                      </TableCell>
                    </TableRow>
                  </TableFooter>
                </Table>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white/70 rounded-lg p-2">
                      ...
                      <div className="flex items-center space-x-1">
                        <p className="font-semibold text-gray-900">{order.truck.licencePlate}</p>
                        <Button
                          variant="ghost"
                          size="sm"
              ...
              <CardContent className="space-y-4">
                <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                  <div className="flex items-center space-x-3 mb-3">
                    ...
                    <div>
                      <h3 className="font-bold text-gray-900">Unit #{order.trailer.unitNumber}</h3>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white/70 rounded-lg p-2">
                      <p className="text-xs font-medium text-gray-600">License Plate</p>
                      <div className="flex items-center space-x-1">
                        <p className="font-semibold text-gray-900">{order.trailer.licencePlate}</p>
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-4 w-4 p-0"
                    ...
                    <div className="bg-white/70 rounded-lg p-2 col-span-2">
                      <p className="text-xs font-medium text-gray-600">VIN</p>
                      <div className="flex items-center space-x-1">
                        <p className="font-semibold text-gray-900 text-xs">{order.trailer.vin}</p>
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-4 w-4 p-0"
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {order.carrierNumbers.mcNumber && (
                        <div className="bg-gray-50 rounded-lg p-3">
                          ...
                          <div className="flex items-center space-x-1">
                            <p className="font-semibold text-gray-900">{order.carrierNumbers.mcNumber}</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-4 w-4 p-0"
                              onClick={() => copyToClipboard(order.carrierNumbers!.mcNumber!)}
                            >
                              <Copy className="w-3 h-3" />
                            </Button>
                          </div>
                      ...
                      {order.carrierNumbers.dotNumber && (
                        <div className="bg-gray-50 rounded-lg p-3">
                          ...
                          <div className="flex items-center space-x-1">
                            <p className="font-semibold text-gray-900">{order.carrierNumbers.dotNumber}</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-4 w-4 p-0"
                              onClick={() => copyToClipboard(order.carrierNumbers!.dotNumber!)}
                            >
                              <Copy className="w-3 h-3" />
                            </Button>
                          </div>
                      ...
                      {order.carrierNumbers.einNumber && (
                        <div className="bg-gray-50 rounded-lg p-3">
                          ...
                          <div className="flex items-center space-x-1">
                            <p className="font-semibold text-gray-900">{order.carrierNumbers.einNumber}</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-4 w-4 p-0"
                              onClick={() => copyToClipboard(order.carrierNumbers!.einNumber!)}
                            >
                              <Copy className="w-3 h-3" />
                            </Button>
                          </div>
                      ...
                      {order.carrierNumbers.iftaNumber && (
                        <div className="bg-gray-50 rounded-lg p-3">
                          <p className="text-xs font-medium text-gray-600 mb-1">IFTA Number</p>
                          <div className="flex items-center space-x-1">
                            <p className="font-semibold text-gray-900">{order.carrierNumbers.iftaNumber}</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-4 w-4 p-0"
                              onClick={() => copyToClipboard(order.carrierNumbers!.iftaNumber!)}
                            >
                              <Copy className="w-3 h-3" />
                            </Button>
                          </div>
                      ...
                      {order.carrierNumbers.orNumber && (
                        <div className="bg-gray-50 rounded-lg p-3">
                          ...
                          <div className="flex items-center space-x-1">
                            <p className="font-semibold text-gray-900">{order.carrierNumbers.orNumber}</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-4 w-4 p-0"
                              onClick={() => copyToClipboard(order.carrierNumbers!.orNumber!)}
                            >
                              <Copy className="w-3 h-3" />
                            </Button>
                          </div>
                      ...
                      {order.carrierNumbers.kyuNumber && (
                        <div className="bg-gray-50 rounded-lg p-3">
                          ...
                          <div className="flex items-center space-x-1">
                            <p className="font-semibold text-gray-900">{order.carrierNumbers.kyuNumber}</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-4 w-4 p-0"
                              onClick={() => copyToClipboard(order.carrierNumbers!.kyuNumber!)}
                            >
                              <Copy className="w-3 h-3" />
                            </Button>
                          </div>
                      ...
                      {order.carrierNumbers.txNumber && (
                        <div className="bg-gray-50 rounded-lg p-3">
                          ...
                          <div className="flex items-center space-x-1">
                            <p className="font-semibold text-gray-900">{order.carrierNumbers.txNumber}</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-4 w-4 p-0"
                              onClick={() => copyToClipboard(order.carrierNumbers!.txNumber!)}
                            >
                              <Copy className="w-3 h-3" />
                            </Button>
                          </div>
                      ...
                      {order.carrierNumbers.tnNumber && (
                        <div className="bg-gray-50 rounded-lg p-3">
                          ...
                          <div className="flex items-center space-x-1">
                            <p className="font-semibold text-gray-900">{order.carrierNumbers.tnNumber}</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-4 w-4 p-0"
                              onClick={() => copyToClipboard(order.carrierNumbers!.tnNumber!)}
                            >
                              <Copy className="w-3 h-3" />
                            </Button>
                          </div>
                      ...
                      {order.carrierNumbers.laNumber && (
                        <div className="bg-gray-50 rounded-lg p-3">
                          ...
                          <div className="flex items-center space-x-1">
                            <p className="font-semibold text-gray-900">{order.carrierNumbers.laNumber}</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-4 w-4 p-0"
                              onClick={() => copyToClipboard(order.carrierNumbers!.laNumber!)}
                            >
                              <Copy className="w-3 h-3" />
                            </Button>
                          </div>
                    ...
                    {order.carrierNumbers.notes && (
                      <div className="mt-4">
                        ...
                        <p className="text-sm text-gray-900 bg-gray-50 rounded-lg p-3">{order.carrierNumbers.notes}</p>
                      </div>
                    )}
                        ...
                        <div className="space-y-2">
                          {order.carrierNumbers.files.map((file, index) => (
                            <div key={index} className="flex items-center justify-between p-2 border rounded">
                              ...
                              <Button variant="ghost" size="sm" onClick={() => downloadCarrierFile(file.filename)}>
                    ...
                    <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="font-semibold text-gray-900">{order.companyInfo.name}</p>
                        </div>
                        {order.companyInfo.dba && (
                          <div>
                            <p className="text-xs font-medium text-gray-600 mb-1">DBA</p>
                            <p className="font-semibold text-gray-900">{order.companyInfo.dba}</p>
                          </div>
                        )}
                      </div>
                      <div>
                        ...
                        <p className="font-semibold text-gray-900">
                          {order.companyInfo.address}, {order.companyInfo.city}, {order.companyInfo.state}{' '}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="flex items-center space-x-1">
                            <p className="font-semibold text-gray-900">{order.companyInfo.phone}</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-4 w-4 p-0"
                        ...
                        {order.companyInfo.fax && (
                          <div>
                            ...
                            <div className="flex items-center space-x-1">
                              <p className="font-semibold text-gray-900">{order.companyInfo.fax}</p>
                              <Button
                                variant="ghost"
                                size="sm"
                                className="h-4 w-4 p-0"
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-1">
                          <p className="font-semibold text-gray-900">{order.companyInfo.email}</p>
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-4 w-4 p-0"
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-900">{new Date(order.updatedAt).toLocaleDateString()}</span>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="space-y-2">
                {order.files.map((file, index) => (
                  <div key={index} className="flex items-center justify-between p-2 border rounded">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => downloadOrderFile(order.id, file.filename)}>
                      <Download className="w-4 h-4" />
                    </Button>
                  </div>
                ))}
