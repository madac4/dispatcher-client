    ...
    <>
      {!haveEntities() && (
        <Alert className="border-red-200 bg-red-50">
          ...
          <AlertDescription className="text-red-800 font-medium text-base">
            You don&apos;t have any {activeTab} yet. You can add one by clicking the button below.
          </AlertDescription>
        </Alert>
      )}
      ...
      <Card>
        <CardHeader className="text-center space-y-2">
          ...
          <CardTitle className="text-3xl font-bold">Fleet Inventory</CardTitle>
          <p className="text-gray-600">Manage your trucks and trailers</p>
        </CardHeader>
          ...
          <Tabs defaultValue={activeTab} className="space-y-6">
            <TabsList className="grid w-full grid-cols-2 h-12 max-w-xl mx-auto">
              {InventoryTabs.map(tab => (
                ...
                >
                  <DynamicIcon name={tab.icon as IconName} className="h-4 w-4" />
                  <span>{tab.label}</span>
                </TabsTrigger>
              ))}
            </TabsList>
