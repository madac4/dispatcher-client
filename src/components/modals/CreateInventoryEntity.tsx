...
export function CreateInventoryEntity({
  ...
  const [nhtsaData, setNhtsaData] = useState<TruckPayload | TrailerPayload | null>(null)
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>Add a new {type} to your inventory.</DialogDescription>
        </DialogHeader>
        ...
        {!nhtsaData && (
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  className={vinError ? 'border-destructive pr-10' : 'pr-10'}
                  maxLength={17}
                />
                {vin.length > 0 && !vinError && vin.length === 17 && (
                  <CheckCircle className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-green-500" />
                )}
                {vinError && <XCircle className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-destructive" />}
              </div>
              {vinError && <p className="text-destructive text-sm">{vinError}</p>}
              <p className="text-xs text-muted-foreground">
                Vehicle Identification Number (VIN) is a 17-character code used to uniquely identify motor vehicles.
              </p>
            </div>
            ...
            <DialogFooter className="justify-end">
              ...
              <Button type="submit" disabled={!vin || vin.length !== 17 || !!vinError || isValidating || isLoading}>
                {isLoading ? (
                  <>
