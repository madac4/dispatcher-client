...
import { deleteCarrierFile, downloadCarrierFile, getCarrierFiles } from '@/lib/services/settingsService'
...
export default function FilesModal({
  ...
  const handleDownloadFile = useCallback(async (filename: string) => {
    const { message } = await downloadCarrierFile(filename)
    toast.success(message)
  }, [])
          ...
          {!isFetching &&
            ...
            carrierFiles?.map(file => (
              <div key={file.filename} className="flex items-center gap-2 p-4 border rounded-lg">
                ...
                <ConfirmModal onConfirm={() => handleDeleteFile(file.filename)} title="Delete File">
