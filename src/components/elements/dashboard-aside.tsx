...
import { getAsideNavigation } from '@/constants/constants';
...
export function DashboardAside() {
  ...
  const navigation = getAsideNavigation(role() === UserRole.ADMIN);
  ...
  return (
    <aside className="flex h-full max-h-screen border-r flex-col bg-background">
      <div className="mb-2 flex min-h-14 items-center justify-between border-b px-4 lg:min-h-16">
        ...
        <Button variant="ghost" size="icon-sm" className="text-gray-600" onClick={toggleAside}>
          {isAsideOpen ? <PanelLeftClose className="size-5" /> : <PanelRightClose className="size-5" />}
        </Button>
      </div>

      <div className="hide-scrollbar flex-1 overflow-y-auto">
        {width >= 768 && navigation && (
          <nav className={cn('space-y-1 px-2 text-sm font-medium text-gray-700 lg:px-4', !isAsideOpen && '!px-4')}>
            {navigation.map((item) => {
                  ...
                  <TooltipTrigger className="w-full" asChild>
                    <Link
                      ...
                      className={cn(
                        ...
                        !isAsideOpen && 'justify-center p-0 h-9 w-9',
                        item.disabled && 'opacity-50 cursor-not-allowed pointer-events-none',
                      )}
                    >
                      <DynamicIcon name={item.icon || 'octagon'} size={20} />
                      {isAsideOpen && item.label}
                    </Link>
                  </TooltipTrigger>
                  <TooltipContent side="right" className={cn(isAsideOpen && 'hidden')}>
                    {item.label}
                  </TooltipContent>
                </Tooltip>
              );
