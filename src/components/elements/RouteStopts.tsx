...
export default function RoutePlanner() {
  ...
  useEffect(() => {
    ...
    const dr = new google.maps.DirectionsRenderer({ draggable: true, map: m })
    ...
    google.maps.event.addListener(dr, 'directions_changed', async () => {
      ...
      // 🚩 dragged stops
      const viaWaypoints = route.via_waypoints ?? []
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex gap-2">
        <input placeholder="Origin Address" {...form.register('origin')} className="border p-2 w-1/2" />
        <input placeholder="Destination Address" {...form.register('destination')} className="border p-2 w-1/2" />
        <button onClick={calculateRoute} className="bg-blue-500 text-white px-4 rounded">
          Calculate
        </button>
      </div>
