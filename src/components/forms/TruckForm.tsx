...
import { createTruck, updateTruck } from '@/lib/services/truckService'
...
type TruckFormProps = {
  nhtsaData: TruckPayload & { _id?: string }
  closeModal: () => void
...
export default function TruckForm({ nhtsaData, closeModal, action }: TruckFormProps) {
  const [isLoading, setIsLoading] = useState(false)
  const { setUpdatedTruck } = useTrucksStore()
  ...
  function onSubmit(values: z.infer<typeof truckSchema>) {
    setIsLoading(true)
    if (action === DialogActions.Create) addTruck(values)
    if (action === DialogActions.Edit) editTruck(values)
  }

  const addTruck = async (values: z.infer<typeof truckSchema>) => {
    try {
  ...
  const editTruck = async (values: z.infer<typeof truckSchema>) => {
    try {
      const response = await updateTruck({ ...values, nrOfAxles: Number(values.nrOfAxles) }, nhtsaData._id || '')
      toast.success(response.message)
      setUpdatedTruck(response.data)
      closeModal()
    } finally {
      setIsLoading(false)
    }
