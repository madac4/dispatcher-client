...
export function PageTabs({ tabs, activeTab, onTabChange, className, tabsListClassName }: PageTabsProps) {
  return (
    <Tabs value={activeTab} defaultValue={tabs[0].value} onValueChange={onTabChange} className={className}>
      <TabsList className={tabsListClassName || 'grid w-full grid-cols-5 h-10 bg-gray-100 p-1 rounded-lg'}>
        {tabs.map((tab) => (
      ...
      {tabs.map((tab, index) => (
        <TabsContent value={tab.value} key={tab.value + index} className="mt-6">
          {tab.content}
        </TabsContent>
      ))}
    </Tabs>
